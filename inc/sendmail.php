...
function sendInvite($to, $type, $amb, $site) {
  ...
  if ($type == 'amb') {
    ...
    $subject = 'You’re invited to the Morning Chalk Up Ambassadors';
    $from = 'Eric Sherred <info@mail.morningchalkup.com>';
  } elseif ($type == 'sub') {
    $copy = getInviteMCU($amb->getValue('full-name'), $amb->getValue('username'), $site);
    $subject = 'Wanna join?';
    $from = 'Eric at Morning Chalk Up <info@mail.morningchalkup.com>';
  }
