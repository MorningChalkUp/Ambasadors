...
function sendInvite($to, $type, $amb) {
  $mg = Mailgun::create(MAILGUN_KEY);
  $domain = MAILGUN_DOMAIN;
  
  if ($type == 'amb') {
    $copy = getInviteAmb($amb->getValue('fname'), $amb->getValue('username'));
    $subject = 'Join Me As A Morning Chalk Up Ambassadors';
  } elseif ($type == 'sub') {
    $copy = getInviteMCU($amb->getValue('full-name'), $amb->getValue('username'));
